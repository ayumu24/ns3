...
#include "ndaodv-routing-protocol.h"
#include "ns3/log.h"
...
#include "ns3/node-list.h" // PAODV 
#include "ns3/ipv4-list-routing.h" // PAODV
#include "ns3/double.h" // PAODV parameters p1 and p2
...
NS_LOG_COMPONENT_DEFINE ("NDAodvRoutingProtocol");
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    .AddAttribute ("p1", "Probability P1 for PAODV.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&RoutingProtocol::m_p1),
                   MakeDoubleChecker<double> (0.0, 1.0))
    .AddAttribute ("p2", "Probability P12for PAODV.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&RoutingProtocol::m_p2),
                   MakeDoubleChecker<double> (0.0, 1.0))
    .AddAttribute ("Linear", "Linear dependance of probability between p1 and p2.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&RoutingProtocol::m_linear),
                   MakeBooleanChecker ())
    .AddAttribute ("PkAodvEnable", "Probability for RREQ retransmit is optimal to chose the best of up to K links from one node.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&RoutingProtocol::m_pkAodvEnable),
                   MakeBooleanChecker ())
    .AddAttribute ("K", "Number of links to choose from.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&RoutingProtocol::m_kLinks),
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  //---------------------------------------- PAODV protocol ----------------------------------------------
  //------------------------------------------------------------------------------------------------------
  NS_ASSERT_MSG (m_p1 >= 0.0, "Probability parameter must fulfill the condition: P1 >= 0");
  NS_ASSERT_MSG (m_p1 <= 1.0, "Probability parameter must fulfill the condition: P1 <= 1");
  NS_ASSERT_MSG (m_p2 >= 0.0, "Probability parameter must fulfill the condition: P2 >= 0");
  NS_ASSERT_MSG (m_p2 <= 1.0, "Probability parameter must fulfill the condition: P2 <= 1");
  NS_ASSERT_MSG (m_p1 >= m_p2, "Probability parameters must fulfill the condition: P1 >= P2");
  
  uint32_t nNodes = NodeList::GetNNodes ();
  uint32_t nNeighbors = 0;
  uint32_t nNeighborsSum = 0;
  uint32_t nNeighborsMax = 0;
  Ptr<Node> node;
  for (uint32_t n = 0; n < nNodes; n++)
    {
       node = NodeList::GetNode (n);
       Ptr<Ipv4> ipv4 = node->GetObject<Ipv4> ();
       NS_ASSERT_MSG (ipv4 != 0, "ipv4 pointer is NULL");
       Ptr<Ipv4RoutingProtocol> routing = ipv4->GetRoutingProtocol ();
       NS_ASSERT_MSG (routing != 0, "routing pointer is NULL");
       Ptr<Ipv4ListRouting> list = DynamicCast<Ipv4ListRouting> (routing);
       NS_ASSERT_MSG (list != 0, "list of routing protocols is NULL");
       int16_t priority;
       Ptr<aodv::RoutingProtocol> aodv = DynamicCast<aodv::RoutingProtocol> (list->GetRoutingProtocol (0, priority));
       NS_ASSERT_MSG (aodv != 0, "aodv pointer is NULL");
       nNeighbors = aodv->GetNumberOfNeighbors ();
       nNeighborsMax = (nNeighborsMax >= nNeighbors) ? nNeighborsMax : nNeighbors;
       nNeighborsSum += nNeighbors;
    }
  double nNeigborsAvg = (double)nNeighborsSum / (double)nNodes;
  
  double prob;
  if (m_linear)
    { // linear version of PAODV
      prob = 1.0 - (double)GetNumberOfNeighbors () / (double)nNeighborsMax;
      prob = (prob > m_p1) ? m_p1 : prob;
      prob = (prob < m_p2) ? m_p2 : prob;
    }
  else
    { // classic PAODV
      prob = m_p1; // for sparce networks
      if (GetNumberOfNeighbors () > nNeigborsAvg)
        {
          prob = m_p2; // for dense networks
        }
    }      

  //-------------------------------------     PK-AODV version protocol   -------------------------------------------
  if (m_pkAodvEnable)
    {
      prob = 1.0; 
      if (GetNumberOfNeighbors () > 1) // if less then or equal to only one neighbor, then probability for RREQ retransmission should be 1. 
        {
          prob = m_kLinks / GetNumberOfNeighbors (); // if there are more neighbors, probability for RREQ retransmission is such that in average m_kLinks are used for route finding process. 
          if (prob > 1.0)
            {
              prob = 1.0;
            }
  ...
  //------------------------------------- end of PAODV version protocol -------------------------------------------- 
    
  double rnd = m_uniformRandomVariable->GetValue (0,1);
  //NS_LOG_UNCOND ("NB = " << GetNumberOfNeighbors () << ", <NB>" << nNeigborsAvg << ", P1 = " << m_p1 << ", P2 = " << m_p2 << ", P = " << prob << ", RND = " << rnd);
  if (rnd > prob) // drop RREQ packet
    {
      //NS_LOG_UNCOND ("RREQ dropped!");
      return;
    }
  ///------------------------------------- end of PAODV protocol -------------------------------------------- 
